...
export function createValidationPrhase({
  ...
  const artCertificateHash = createHmac("sha256", secret)
    ...
    .digest("hex");
  const createPINHalf = artCertificateHash.substring(
    artCertificateHash.length - PIN_CONDE_LENGTH
  );
  ...
  const createPIN2Half = pinHashHalf.substring(
    pinHashHalf.length - PIN_CONDE_LENGTH
  );
  const pinToArt = convertToBase64URISecure(
    `${createPINHalf}${createPIN2Half}`
  );
...
export async function checkValidationPrhase({
  ...
  const certificateValidationPhraseParts =
    certificateValidationPhrase.split(".");
  ...
  const createPINHalf = certificatePinHash.substring(
    certificatePinHash.length - PIN_CONDE_LENGTH
  );
  const createPIN2Half = certificatePinHashHalf.substring(
    certificatePinHashHalf.length - PIN_CONDE_LENGTH
  );

  const pinGeneratedFromHash = convertToBase64URISecure(
    `${createPINHalf}${createPIN2Half}`
  );
